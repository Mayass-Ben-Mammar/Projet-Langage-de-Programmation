    ...
    return fetch("https://restcountries.com/v3.1/all")
        ...
        .catch(error => {
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
                text: 'Impossible de récupérer les pays. Vérifie ta connexion Internet.'
            });
            throw error;
        });
}
...
function Commencer(){
    obtenirTousLesPays().then(data => {
        ...
        // 🔥 Nouvelle partie: récupérer pays mystère ou en choisir un
        const paysMystere = localStorage.getItem("paysMystere");

        if (paysMystere) {
            try {
                choix = JSON.parse(paysMystere);
                console.log("Pays mystère récupéré du cache :", choix.name.common);
            } catch (e) {
                console.error("Erreur de lecture pays mystère, en choisissant un nouveau.");
                choix = paysdata[Math.floor(Math.random() * paysdata.length)];
                localStorage.setItem("paysMystere", JSON.stringify(choix));
            }
        } else {
            choix = paysdata[Math.floor(Math.random() * paysdata.length)];
            localStorage.setItem("paysMystere", JSON.stringify(choix));
            console.log("Nouveau pays mystère :", choix.name.common);
        }
    ...
    if (feur) {
        ...
        console.error("Erreur de sélection de l'élément blanc");
    }
}
...
function Devine() {
    ...
    if (cdt == true) {
        ...
        document.getElementById("Entree").innerHTML = "";
        vie = 6;
        localStorage.removeItem("paysMystere"); // 🔥 on enlève le pays du cache quand gagné
        return;
    } else {
        vie--;
        ...
        if (vie == 0) {
            ...
            document.getElementById("Entree").innerHTML = "";
            localStorage.removeItem("paysMystere"); // 🔥 on enlève aussi si perdu
            return;
        }
    }
}
...
function deletecache() {
    localStorage.removeItem("paysData");
    localStorage.removeItem("paysData_exp");
    localStorage.removeItem("paysMystere"); // 🔥 vider aussi le pays mystère
}
